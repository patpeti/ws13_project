...
public class DECOMPForecaster {
	...
	private List<Double> decomp(List<Double> reduced, Integer horizon) {
			...
			try {
				...
				re.eval("x2 <- stl(ts(x1,start=0,frequency="+frequency+")[,1], t.window="+reducedSize+", s.window=\"periodic\", robust=TRUE)");
				re.eval("x3 <- data.frame(forecast(x2,method=\"naive\",h="+horizon+"))");
