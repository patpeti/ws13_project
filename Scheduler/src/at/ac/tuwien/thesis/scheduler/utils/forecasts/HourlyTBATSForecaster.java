...
public class HourlyTBATSForecaster {
	...
	private List<Double> hw(List<Double> reduced, Integer horizon) {
			...
			Integer hourfrequency = frequency/24;
			Integer reducedSize = (int) Math.ceil(dataSize/dr_factor);
			System.out.println("***********  DEBUG OUTPUT  ***********");
			System.out.println(" frequency: "+ frequency + " horizon: "+ horizon + " reducedSize: "+ reducedSize+ " weekfreq: " + hourfrequency);
			...
			try {
				...
				System.out.println("x2 <- msts(x1, seasonal.periods=c("+hourfrequency+","+frequency+"), ts.frequency="+frequency+", start=0)");
				re.eval("x2 <- msts(x1, seasonal.periods=c("+hourfrequency+","+frequency+"), ts.frequency="+frequency+", start=0)");
				System.out.println("x3 <- tbats(x2, use.trend=NULL, seasonal.periods=c("+hourfrequency+","+frequency+"), bc.lower=0,bc.upper=100)");
				re.eval("x3 <- tbats(x2, use.trend=NULL, seasonal.periods=c("+hourfrequency+","+frequency+"), bc.lower=0,bc.upper=100)");
