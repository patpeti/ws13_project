...
public class DSHWForecaster {
	...
	private List<Double> hw(List<Double> reduced) {
			...
			Integer horizon = Constants.DataPerWeek/dr_factor;
			...
			try {
				...
				re.eval("x2 <- msts(x1, seasonal.periods=c("+frequency+","+frequency*7+"), ts.frequency="+frequency*7+", start=0)");
